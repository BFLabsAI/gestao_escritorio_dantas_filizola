-- Function para verificar disponibilidade de horário com duração variável
...
BEGIN
  ...
  SELECT json_agg(
    json_build_object(
      ...
      'end_time', a.end_time,
      'status', a.status,
      'patient_name', COALESCE(pac.nome_completo, 'Paciente'),
  ...
  LEFT JOIN (
    SELECT id, nome_completo FROM leads_agenda_inteligente_orus WHERE status = 'convertido'
  ) pac ON a.patient_id = pac.id
  ...
  RETURN json_build_object(
    ...
    'dia_semana', to_char(p_data, 'Day', 'pt_BR')
  );
END;
$$ LANGUAGE plpgsql;

-- Adicionar comentário
COMMENT ON FUNCTION verificar_disponibilidade_horario_flexivel_agenda_inteligente_orus IS 'Verifica disponibilidade de horário considerando duração variável do procedimento';;
